...
void PageLibPreprocessor::buildInvertIndexTable()
{
	...
	{	
		int df = item.second.size();              //每个词的所在的文章的个数和
		//求关键词item.first的逆文档频率
		double idf = log(static_cast<double>(totalPageNum)/ df + 0.05) / log(2);
		
		for(auto & sitem : item.second)
		//对vec<int,duble>进行遍历，int代表文章id，duble代表该单词在这篇文章中的词频
		{
			double weight = sitem.second * idf;   

			weightSum[sitem.first] += pow(weight, 2);  //保存每一篇文章的所有词词的权重的平方和  
			sitem.second = weight;
		}
	}
	...
	{	//归一化处理
		for(auto & sitem : item.second) 
//用每个词在该篇文章中的权重值除以该篇文章所有词权重的平方差，获取最终的权重值，保存在倒排索引中
		{
